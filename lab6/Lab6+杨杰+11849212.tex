...
Finally calculate the correlation image of them. There are a spike in \ffref{fig:conv}{b}(red rectangle). While \ffref{fig:conv}{a} has no. This indicate that there are face in face in background image, this result is correction.
\begin{figure}[H]
\centering
...
In this experiment, we can find that homomorphic filter can remove illuminate light to enhance image. However the hyper parameters of the filter is hard to determine base on the original image. At most condition this is a skillful method.

The bandreject filter can remove noise from image. However, it require noise concentrate in one region. usually, the noise also very to separate form original image. In practice, we often choose notch filter which only block on frequency noise. This filter is a point in frequency rather than a ring.

The correlation can be used to detect whether template in a image. but this method has low accuracy. Usually this correlation will be influenced by background. The difference between negative and positive is too small to distinguish.
